...
async function fireNotifs(userId) {
  ...
  for (const e of db.getAllUserEvents(userId)) {
    const du = daysUntil(e.date);
    if (!thresholds.has(du)) continue;
    const label = du === 0 ? '¡Hoy!' : du === 1 ? 'Mañana' : `En ${du} días`;
    new Notification(`${label} — ${e.title}`, { body: subMap[e.subjectId]?.name ?? '', icon: '/favicon.svg', tag: e.id });
...
export default function NotificationBanner() {
  ...
  useEffect(() => {
    ...
    if (Notification.permission === 'granted') {
      ...
      if (sessionStorage.getItem(key) !== today) { sessionStorage.setItem(key, today); fireNotifs(user.userId); }
    }
  }, [user]);
  ...
  const allow = async () => {
    ...
    setVisible(false);
    if (perm === 'granted' && user) fireNotifs(user.userId);
  };
