...
function runQuery(q, userId) {
  if (!q.trim()) return [];
  const lq = q.toLowerCase();
  const out = [];
  const subjects = db.getAllSubjects(userId);
  ...
  db.getAllUserEvents(userId).forEach(e => {
    if ([e.title, e.description].some(v => v?.toLowerCase().includes(lq))) {
      const du = Math.ceil((new Date(e.date) - new Date()) / 86400000);
      out.push({ type: 'event', title: e.title, sub: `${subMap[e.subjectId]?.name ?? ''} · ${du > 0 ? `en ${du}d` : 'pasado'}`, color: subMap[e.subjectId]?.color, action: () => navigate('/subjects/' + e.subjectId) });
    }
  });
  subjects.forEach(s => {
    db.getNotes(s.id).forEach(n => {
    ...
    db.getSubjectLinks(s.id).forEach(l => {
...
export default function Search({ open, onClose }) {
  ...
  useEffect(() => {
    if (!user || !open) return;
    const r = runQuery(q, user.userId);
    setResults(r);
    ...
    <div className="search-overlay search-open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="search-box">
        ...
        <div className="search-results">
          {!q && <p className="search-empty">Empezá a escribir...</p>}
          {q && !results.length && <p className="search-empty">Sin resultados para "{q}"</p>}
